...
public class flameBoss : MonoBehaviour
{
    ...
    public void shootFlame(){
        List<Vector3> velocities = new List<Vector3>{new Vector3(0, -1, 0),
                                                     new Vector3((float)Math.Sqrt(2), (float)-Math.Sqrt(2), 0) * 1/2,
                                                     new Vector3(1, 0, 0),
                                                     new Vector3((float)Math.Sqrt(2),(float)Math.Sqrt(2), 0) * 1/2,
                                                     new Vector3(0, 1, 0),
                                                     new Vector3((float)-Math.Sqrt(2),(float)Math.Sqrt(2), 0) * 1/2,
        ...
        {
            flame.Create(new Vector3(positions[i].x + firePointCenter.position.x, positions[i].y + firePointCenter.position.y, 0), velocities[i]);
        }
